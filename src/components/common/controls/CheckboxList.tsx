import { Checkbox, FormControlLabel, makeStyles, Grid } from '@material-ui/core';
...
const CheckboxList = ({ list, onChange }: CheckBoxListProps) => {
    ...
    const handleChange = (e, item) => {
        inpList.forEach((el) => (el.value === item.value ? (el.isActive = e.target.checked) : ''));
        setInpList([...list]);
        if (onChange) {
            onChange(e, inpList);
        }
    ...
    const getDesc = (item) => {
        return (
            <div className={'custom-label'}>
                <img src={item.isActive ? item.inActiveIcon : item.icon} alt={item.title} />
            </div>
        );
    };
    return (
